...
ChatLogic::~ChatLogic()
{
    ...
    // TASK 3: Due to unique_ptr deleting is automatically done
    ...
    // TASK 4: Due to unique_ptr deleting is automatically done
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // OLD: auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; });
                        // TASK 3: Graphnode as unique_ptr
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // OLD: _edges.push_back(edge);

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            // TASK 4 get due to unique_ptr
                            (*childNode)->AddEdgeToParentNode(edge.get());
                            // TASK 4 MOVE the edge
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                // OLD: rootNode = *it; // assign current node to root
                rootNode = it->get();
            }
            else
            {
